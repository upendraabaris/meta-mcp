...
export function registerAnalyticsTools(server, metaClient) {
    // Get Insights Tool
    server.tool("get_insights", GetInsightsSchema.shape, async ({ object_id, level, date_preset, time_range, fields, breakdowns, limit, }) => {
        try {
            const params = {
                level,
                limit: limit || 25,
    ...
    server.tool("compare_performance", ComparePerformanceSchema.shape, async ({ object_ids, level, date_preset, time_range, metrics }) => {
        try {
            const params = {
                level,
                fields: metrics,
    ...
    server.tool("export_insights", ExportInsightsSchema.shape, async ({ object_id, level, format, date_preset, time_range, fields, breakdowns, }) => {
        try {
            const params = {
                level,
...
function calculatePerformanceRankings(comparisons, metrics) {
    const rankings = {};
    for (const metric of metrics) {
        ...
        const sorted = validComparisons
            ...
            const isCostMetric = metric.includes("cpc") ||
                metric.includes("cpm") ||
...
function getMetricValue(metrics, metricName) {
    const value = metrics[`total_${metricName}`] ||
        metrics[`average_${metricName}`] ||
        metrics[metricName];
    return value !== undefined ? parseFloat(value) : null;
}
function calculateAttributionMetrics(insights) {
